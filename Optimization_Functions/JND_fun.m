function [c,ceq] = JND_fun(x,R,ideal_xy,cmf,Wavelength,g11,g22,g12,JND_goal,mode,minmax)
    testSPD=R*x';
    ...
    slope=(y-ideal_xy(2))/(x-ideal_xy(1));
    ...
    farther_y=ideal_xy(2)+slope*(farther_x-ideal_xy(1));
    
    
    [ellipse_x,ellipse_y,~,~,~] = macAdamEllipse(ideal_xy(1),ideal_xy(2),3,g11,g22,g12);
    intersection = InterX([ellipse_x';ellipse_y'],[farther_x ideal_xy(1);farther_y ideal_xy(2)]);
    %disp(intersection)
    %[xcross,ycross]=polyxpoly(ellipse_x,ellipse_y,[farther_x-ideal_xy(1)],[farther_y-ideal_xy(2)]);
    JND=sqrt((ideal_xy(1)-intersection(1,1)).^2+(ideal_xy(2)-intersection(2,1)).^2);
    %disp([ideal_xy(1)-intersection(1,1) ideal_xy(2)-intersection(2,1)])
    if strcmp(mode,'constraint')==1
        if minmax(2)==1
            c=-1*(getdE_xy(ideal_xy(1),ideal_xy(2),x,y)/JND-JND_goal);
        else
            c=getdE_xy(ideal_xy(1),ideal_xy(2),x,y)/JND-JND_goal;
        end
    else
        if minmax(1)==1
            c=-1*getdE_xy(ideal_xy(1),ideal_xy(2),x,y)/JND;
        else
            c=getdE_xy(ideal_xy(1),ideal_xy(2),x,y)/JND; 
        end
    end
    ceq=[];
