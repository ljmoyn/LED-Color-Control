...
% for i=1:length(u)
%     if mod(i,5)==1
%         u(i,1)=0.696*u(i,1)^3-0.442*u(i,1)^2+0.706*u(i,1)+.0029;
%     elseif mod(i,5)==2
%         u(i,1)=-0.0587*u(i,1)^3+0.255*u(i,1)^2+0.806*u(i,1)-.014;
%     elseif mod(i,5)==3
%         u(i,1)=-0.028*u(i,1)^3+0.2*u(i,1)^2+0.83*u(i,1)+.0033; 
%     elseif mod(i,5)==4
%         u(i,1)=-0.012*u(i,1)^3+0.16*u(i,1)^2+0.85*u(i,1)+.0022;
%     elseif mod(i,5)==0
%         u(i,1)=0.00553*u(i,1)^3+0.0375*u(i,1)^2+0.953*u(i,1)+.00549;        
%     end
% end
u=65535*u;
